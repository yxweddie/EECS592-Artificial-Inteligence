...
def update_total_cost(queue,node_coordinate,t_cost,health,p_coordinate,p_health,parent):
    for i in range(0, len(queue)):
        n = list(queue[i])
        if int(n[0][0]) == int(node_coordinate[0]) and int(n[0][1]) == int(node_coordinate[1]) and int(health) == int(n[4]) :
            if t_cost < int(n[1]):
                new_node = list(n)
                new_node[1] = t_cost
...
def a_star():
    ...
    parent = dict()
    start_node = create_node(start,0,calculate_heuristic_cost(start,goal),5)
    frontier = list()
